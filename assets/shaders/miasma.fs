...
extern MY_HIGHP_OR_MEDIUMP vec2 miasma;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec2 noise = vec2(
        dot(floor(texture_coords*image_details+miasma.yx*vec2(3.17634,10.186)), vec2(12.9898, 78.233)),
        dot(floor(texture_coords*image_details+miasma.yx*vec2(4.94356,7.234)), vec2(45.2345, 894.2341))
    );
	...
	float alph2 = 0.1;
	qalph -= pow(hsl.z, 4)*0.5;
	hsl.y = pow(hsl.y, 0.5) + pow(hsl.z, 8)*0.5;
	hsl.x = hsl.x * 0.25 + 0.7 * 0.75;
	hsl.z -= pow(hsl.z, 8)*0.1;
	//hsl.z += 0.1;
	//qalph = pow(qalph, 0.5);
